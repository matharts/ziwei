...
test('Rslib rejects type errors even in a source file not imported by the entry', t => {
  ...
  symlinkSync(dirname(packageRequire.resolve('typescript/package.json')), join(directory, 'node_modules/typescript'), 'junction');
  ...
  writeFileSync(join(directory, 'src/invalid.ts'), "export const invalid: number = 'not a number';\n");
  const result = build();
