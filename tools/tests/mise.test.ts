...
const literalArgs = Object.freeze([
  '--name', 'contains spaces', '中文', 'quote\'"', 'literal;$()&',
  '', ' ', '\t', '\n', '\r\n', 'C:\\path with spaces\\file\\', 'line one\nline two',
  ' leading ', '  $ZIWEI_LITERAL  ',
  '$ZIWEI_LITERAL', '${ZIWEI_LITERAL}', '`echo expanded`', '%ZIWEI_LITERAL%',
  '*', '?', '[ab]', '--dir', 'unchanged', '--filter', 'unchanged', '--color', '--', '-C', 'unchanged',
]);
...
type Probe = { version: string; executable: string; child: { version: string; executable: string }; args: string[]; cwd: string };
const rows = (stdout: string): Probe[] => stdout.split('\n').filter(line => line.startsWith('{')).map(line => JSON.parse(line));
...
function fixture(t: TestContext, { activated = true } = {}) {
  ...
  for (const [task, spec] of Object.entries(tasks)) {
    ...
    writeFileSync(join(moduleRoot, 'package.json'), JSON.stringify({ name: spec.package, type: 'commonjs', bin: { [spec.bin]: spec.path } }));
    writeFileSync(cli, `
  ...
  const env: NodeJS.ProcessEnv = { ...process.env, MISE_TRUSTED_CONFIG_PATHS: directory, ZIWEI_LITERAL: 'must not expand' };
  if (!activated) delete env.__MISE_DIFF;
  const pathKey = Object.keys(env).find(key => key.toLowerCase() === 'path') ?? 'PATH';
  env[pathKey] = shadow + delimiter + env[pathKey];
  const run = (args: string[], cwd = directory) => spawnSync('mise', args, {
  ...
  test(`mise tasks select Node and preserve command order (activation record: ${activated})`, t => {
    ...
    for (const [task, expected] of [
      ...
      for (const [i, name] of expected.entries()) {
        ...
        assert.equal(realpathSync.native(output[i].cwd), realpathSync.native(join(directory, tasks[name].cwd)));
      }
    }
  });
}
...
test('mise owns task definitions and CLI paths match installed dependency manifests', () => {
  ...
  for (const spec of Object.values(tasks)) {
    const manifest = JSON.parse(readFileSync(join(root, spec.cwd, 'node_modules', spec.package, 'package.json'), 'utf8'));
    assert.equal(manifest.bin[spec.bin].replace(/^\.\//, ''), spec.path);
  }
  assert.doesNotMatch(readFileSync(join(root, 'mise.toml'), 'utf8'), /tools\/node\/run\.|pnpm (?:--filter \S+ )?run /);
});
...
test('root devEngines owns development constraints and agrees with mise and the consumer floor', () => {
  ...
  assert.deepEqual(manifest.devEngines, {
    ...
    packageManager: { name: 'pnpm', version: /^pnpm = "([^"]+)"$/m.exec(mise)?.[1], onFail: 'error' },
  });
  ...
  assert.doesNotMatch(readFileSync(join(root, 'pnpm-workspace.yaml'), 'utf8'), /^(?:pmOnFail|runtimeOnFail):/m);
});
...
test('the documented direct CLI entry preserves literal arguments and the selected Node', t => {
  ...
  const result = run(['exec', '--', 'pnpm', 'exec', '--', 'node', cli, ...spec.args, ...literalArgs]);
  assert.equal(result.status, 0, result.stderr);
  const output = rows(result.stdout);
...
test('leaf tasks accept ordinary CLI options without invoking aggregate builds', t => {
  const { run } = fixture(t);
  for (const [task, args] of [['test:node', ['-t', 'palace']], ['build:node:ts', ['--watch']]] as const) {
    const result = run(['run', task, '--', ...args]);
    assert.equal(result.status, 0, result.stderr);
    assert.deepEqual(rows(result.stdout).map(row => row.args), [[...tasks[task].args, ...args]]);
  }
});
...
for (const [failed, expected] of [
  ...
  test(`a ${failed} failure preserves diagnostics and stops subsequent checks`, t => {
    ...
    writeFileSync(cliPaths[failed], "process.stdout.write('tool-out\\n'); process.stderr.write('tool-error\\n'); process.exit(23);\n");
    const result = run(['run', 'check:node']);
    assert.equal(result.status, 23);
    assert.match(result.stdout, /tool-out/);
    assert.match(result.stderr, /tool-error/);
    assert.deepEqual(rows(result.stdout).map(row => row.args), expected.map(task => tasks[task].args));
  });
}
