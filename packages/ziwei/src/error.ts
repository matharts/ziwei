...
export type ReceivedValue =
  ...
  | { readonly type: 'undefined' | 'null' | 'boolean' | 'string' | 'bigint'
      | 'symbol' | 'function' | 'object' | 'array' | 'unread' };
...
export function argumentError(
  path: string[], reason: ArgumentFailureReason, value?: unknown,
): ZiweiError {
  return createError({
    code: 'INVALID_ARGUMENT', path, reason,
    received: reason === 'accessor' || reason === 'unknown_field'
      ? { type: 'unread' } : describe(value),
...
function received(failure: NativeFailure): ReceivedValue {
  switch (failure.receivedType) {
    case 'number': return { type: 'number', value: required(failure.value) };
    case 'undefined': case 'null': case 'boolean': case 'string': case 'bigint':
    case 'symbol': case 'function': case 'object': case 'array':
      return { type: failure.receivedType };
    default: throw new Error('原生错误含未知参数类型');
  }
}

function reason(failure: NativeFailure): ArgumentFailureReason {
  switch (failure.reason) {
    case 'type': case 'non_finite': case 'non_integer': case 'out_of_range': case 'not_member':
      return failure.reason;
    default: throw new Error('原生错误含未知参数原因');
  }
}
...
export function nativeError(failure: NativeFailure): ZiweiError {
  switch (failure.code) {
    case 'INVALID_ARGUMENT':
      return createError({
        code: failure.code, path: [required(failure.path)],
        reason: reason(failure), received: received(failure),
      }, failure.message);
    ...
    case 'INVALID_LUNISOLAR_MONTH': case 'INVALID_LUNISOLAR_DAY':
    case 'INVALID_DECADE_INDEX': case 'INVALID_YEARLY_INDEX':
      return createError({ code: failure.code, value: required(failure.value) }, failure.message);
    default: throw new Error('原生错误码不受当前适配包支持');
  }
}
