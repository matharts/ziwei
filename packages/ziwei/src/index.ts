...
export type { Decade, Yearly, DecadeYear, PeriodIndices, DecadeIndex, YearlyIndex } from './types.js';
...
export type { ArgumentFailureReason, ReceivedValue, ZiweiErrorCode, ZiweiErrorDetail } from './error.js';

const birthFields = ['gender', 'birthYear', 'birthMonth', 'birthDay', 'birthHour'];
const parameterFields = ['gender', 'birthStem', 'birthBranch', 'birthMonth', 'ziweiBranch', 'birthHour'];
...
export const Ziwei = Object.freeze({
  ...
  fromParameters(parameters: Parameters): Natal {
    const [gender, stem, branch, month, ziweiBranch, hour] = capture(parameters, parameterFields, arguments.length === 0);
