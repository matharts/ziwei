...
import type { Branch, FiveElementBureau, LocatedStar, Natal, Palace, PalaceName, PalaceTransformation, Profile, Star, StarName, Transformation, Zodiac } from './types.js';
import type { Decade, DecadeIndex, DecadeYear, NatalSnapshot, PeriodIndices, Yearly, YearlyIndex } from './types.js';
...
class NatalHandle implements Natal {
  ...
  toJSON(): NatalSnapshot {
    ...
    return Object.freeze({
      profile: this.profile, zodiac: this.zodiac, fiveElementBureau: this.fiveElementBureau,
      palaces: this.palaces,
      mingPalaceBranch: locations.mingPalaceBranch, shenPalaceBranch: locations.shenPalaceBranch,
      originPalaceBranch: locations.originPalaceBranch, ziweiBranch: locations.ziweiBranch,
    });
  }
  ...
  decadeYears(decade: DecadeIndex): readonly DecadeYear[] {
    ...
    return Object.freeze(unwrap(this.#native.decadeYears(decade)).map(raw =>
  ...
  mingPalace(): Palace { return palace(this.#native.mingPalace()); }
  shenPalace(): Palace { return palace(this.#native.shenPalace()); }
  originPalace(): Palace { return palace(this.#native.originPalace()); }
  ziweiPalace(): Palace { return palace(this.#native.ziweiPalace()); }
  ...
  palaceTransformationSources(targetBranch: Branch): readonly PalaceTransformation[] {
    ...
    return Object.freeze(unwrap(this.#native.palaceTransformationSources(targetBranch)).map(relation));
  }
  ...
  get profile(): Profile {
    ...
    const base = {
      gender: raw.gender, birthStem: raw.birthStem, birthBranch: raw.birthBranch,
      birthMonth: raw.birthMonth, birthHour: raw.birthHour,
    };
    ...
    if (raw.birthYear == null && raw.birthDay == null) {
      ...
      this.#profile = Object.freeze({ ...base, birthYear: raw.birthYear, birthDay: raw.birthDay });
    } else {
...
function star(raw: native.NativeStar): Star {
  ...
  const [name, nameHans, nameHant, abbrHans, abbrHant, category, galaxy,
    birthTransformation, inward, outward] = raw;
  return Object.freeze({
    name, nameHans, nameHant, abbrHans, abbrHant, category, galaxy,
    birthTransformation,
...
function palace(raw: native.NativePalace): Palace {
  ...
  return Object.freeze({
    name, nameHans, nameHant, branch, stem,
...
function relation(raw: native.NativePalaceTransformation): PalaceTransformation {
  return Object.freeze({
    sourceBranch: raw.sourceBranch, targetBranch: raw.targetBranch,
    transformation: raw.transformation, star: raw.star,
  });
}
