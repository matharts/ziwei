...
function contractFingerprint() {
  return fingerprint(
    packageRoot,
    ["run.ts", "suite.ts", "record.ts"].map((name) => ({ path: `bench/${name}` })),
  );
}
...
function command(
  ...
  const stdout = openSync(
    join(output, `${label}.${label === "run" ? "jsonl" : "stdout.log"}`),
    "wx",
  );
...
async function main(config: Options) {
  ...
  if (config.output) {
    ...
    try {
      ...
      if (error instanceof Error && "code" in error && error.code === "EEXIST")
        throw new Error(`输出目录已存在：${output}`);
      throw error;
    }
  } else {
  ...
  try {
    ...
    const record = {
      schemaVersion: 1,
      status: config.smoke ? "smoke" : "provisional",
      startedAt,
      ...
      fingerprints: { source, artifact, contract },
      commands,
      toolchain,
    };
    stage = "output";
