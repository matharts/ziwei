...
test("public package smoke records complete samples and protects existing output", (t) => {
  ...
  for (const sample of record.samples) {
    ...
    assert.equal(
      record.metrics[sample.name].p95BatchMeanNsPerOp,
      sample.elapsedNs / sample.operations,
    );
  }
  ...
  assert.deepEqual(
    lines.filter((line) => line.type === "sample").map(({ type: _type, ...sample }) => sample),
    record.samples,
  );
  ...
  const sample = (rows: RecordRow[], index = 1) => {
    const row = rows[index];
    assert.ok(row.type === "sample");
    return row;
  };
  const start = (rows: RecordRow[]) => {
    const row = rows[0];
    assert.ok(row.type === "start");
    return row;
  };
  const complete = (rows: RecordRow[]) => {
    const row = rows.at(-1);
    assert.ok(row?.type === "complete");
    return row;
  };
  const damaged: ((rows: RecordRow[]) => void)[] = [
    ...
    (rows) => {
      sample(rows).operations++;
    },
    (rows) => {
      sample(rows).elapsedNs = 0;
    },
    (rows) => {
      sample(rows).elapsedNs = 1.5;
    },
    (rows) => {
      sample(rows).elapsedNs = Number.MAX_SAFE_INTEGER + 1;
    },
    (rows) => {
      sample(rows, 2).name = sample(rows).name;
    },
    (rows) => {
      start(rows).protocol.warmup++;
    },
    (rows) => {
      complete(rows).corpusHash = "different";
    },
    (rows) => {
      start(rows).runtime.v8 = "different";
    },
  ];
  for (const corrupt of damaged) {
    ...
    corrupt(rows);
    assert.throws(() =>
      parseRecord(rows.map((row) => JSON.stringify(row)).join("\n"), protocol(true)),
    );
  }
...
function fixture(t: TestContext) {
  ...
  const run = (options: Partial<SpawnSyncOptionsWithStringEncoding> = {}) =>
    spawnSync(process.execPath, [join(pkg, "bench/run.ts"), "--smoke", "--output", output], {
      cwd: pkg,
      encoding: "utf8",
      timeout: 120_000,
      ...options,
...
function builtFixture(t: TestContext, childBody: string) {
  ...
  const calls = original.match(
    /await\s+measure\(\s*process\.argv\[2\]\s*===\s*(["'])--smoke\1\s*\);/g,
  );
  assert.ok(calls?.length === 1, "Expected exactly one measurement entry point");
  const call = calls[0];
...
describe("child failures, malformed output and changed files cannot become valid records", () => {
  for (const { name, body, stage, raw } of [
    {
      name: "child error",
      body: "process.stdout.write('partial raw\\n'); throw new Error('fixture child failure');",
      stage: "measure",
      raw: /partial raw/,
    },
    {
      name: "malformed record",
      body: "process.stdout.write('{broken\\n');",
      stage: "validate",
      raw: /broken/,
    },
    {
      name: "source mutation",
      body: "$MEASURE (await import('node:fs')).appendFileSync(new URL('../src/index.ts', import.meta.url), '\\n// fixture mutation\\n');",
      stage: "validate",
      raw: /complete/,
    },
    {
      name: "build config mutation",
      body: "$MEASURE (await import('node:fs')).appendFileSync(new URL('../rslib.config.ts', import.meta.url), '\\n// fixture mutation\\n');",
      stage: "validate",
      raw: /complete/,
    },
    {
      name: "task definition mutation",
      body: "$MEASURE (await import('node:fs')).appendFileSync(new URL('../../../mise.toml', import.meta.url), '\\n# fixture mutation\\n');",
      stage: "validate",
      raw: /complete/,
    },
    {
      name: "artifact mutation",
      body: "$MEASURE (await import('node:fs')).appendFileSync(new URL('../dist/index.js', import.meta.url), '\\n// fixture mutation\\n');",
      stage: "validate",
      raw: /complete/,
    },
  ]) {
    test(name, (t) => {
      ...
      if (name === "child error")
        assert.match(readFileSync(join(output, "run.stderr.log"), "utf8"), /fixture child failure/);
...
test.skipIf(process.platform === "win32")(
  "closed stdout does not leave a success record",
  async (t) => {
    const { output, pkg, env } = builtFixture(t, "$MEASURE");
    const child = spawn(
      process.execPath,
      [join(pkg, "bench/run.ts"), "--smoke", "--output", output],
      {
        cwd: pkg,
        env,
...
test.skipIf(process.platform === "win32")(
  "build and measurement preserve the selected Node runtime despite PATH shadowing",
  (t) => {
    ...
    const result = run({
      env: { ...process.env, PATH: `${dirname(shadow)}:${process.env.PATH}` },
    });
    assert.equal(result.status, 0, result.stderr);
