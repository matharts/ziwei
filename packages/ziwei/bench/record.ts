...
type Metrics = Record<
  string,
  { medianNsPerOp: number; p95BatchMeanNsPerOp: number; batchCount: number }
>;
...
export function parseRecord(raw: string, plan: Protocol) {
  ...
  const samples = lines.slice(1, -1).map((row, index) => {
    ...
    const { type: _type, elapsedNs, ...identity } = row;
  ...
  for (const { name } of plan.cases) {
    const values = samples
      ...
      .map((sample) => sample.elapsedNs / sample.operations)
      .sort((a, b) => a - b);
