...
export function protocol(smoke: boolean) {
  return {
    ...
    rounds: smoke ? 1 : 3,
    batches: smoke ? 1 : 7,
    warmup: smoke ? 512 : 2560,
    gcPasses: 3,
    rssLimitBytes: 1024 ** 3,
    cases: Object.entries(counts).map(([name, operations]) => ({
      name,
      operations: smoke ? 512 : operations,
    })),
  };
}
...
function resourceState() {
  return { memory: process.memoryUsage(), freeMemory: freemem(), loadAverage: loadavg() };
}
...
async function measure(smoke: boolean) {
  ...
  const charts = [
    ...births.map((x) => Ziwei.fromBirth(x)),
    ...parameters.map((x) => Ziwei.fromParameters(x)),
  ];
  // Validate and pre-materialize the read corpus outside all timed batches.
  for (const chart of charts) {
    assert.equal(chart.palaces.length, 12);
    assert.equal(
      chart.palaces.reduce((sum, palace) => sum + palace.stars.length, 0),
      18,
    );
  ...
  for (const sample of sampleOrder(plan)) {
    ...
    if (sample.batch === 0) {
      ...
      for (let pass = 0; pass < plan.gcPasses; pass++) {
        global.gc();
        await setImmediate();
      }
    }
  ...
  emit({
    type: "complete",
    corpusHash,
    samples: sampleOrder(plan).length,
    after: resourceState(),
  });
}
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    assert.ok(
      process.argv.length === 2 || (process.argv.length === 3 && process.argv[2] === "--smoke"),
    );
