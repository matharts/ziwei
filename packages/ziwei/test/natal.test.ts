  ...
  for (const natal of [
    ...
    assert.deepEqual(
      natal.palaces.map((p) => [
        ...
        p.stem,
        p.decadeAgeRange,
        p.stars.map((s) => s.name),
      ]),
      expectedPalaces,
    );
    const located = new Map<string, { branch: Branch; star: Star }>(
      natal.palaces.flatMap((p) =>
        p.stars.map((s) => [s.name, { branch: p.branch, star: s }] as const),
      ),
    );
    ...
    for (const [name, branch, birth, inward, outward] of fixture) {
      ...
      assert.deepEqual(star, {
        name,
        nameHans: metadata[name][0],
        nameHant: metadata[name][1],
        abbrHans: metadata[name][2],
        abbrHant: metadata[name][3],
        category: metadata[name][4],
        galaxy: metadata[name][5],
...
test("palace snapshots are deeply readonly, stable per chart and detached from chart and input", () => {
  const birth: { -readonly [K in keyof Birth]: Birth[K] } = {
    gender: 1,
    birthYear: 1984,
    birthMonth: 1,
    birthDay: 6,
  ...
  for (const palace of palaces) {
    ...
    for (const value of [palace, palace.stars, palace.decadeAgeRange])
  ...
  assert.throws(() => {
    // @ts-expect-error Deliberately test a frozen output write at runtime.
    palaces[0].decadeAgeRange[0] = 99;
  }, TypeError);
  assert.throws(() => {
    // @ts-expect-error Deliberately test a frozen output write at runtime.
    palaces[0].stars[0].birthTransformation = "A";
  }, TypeError);
  assert.throws(() => {
    // @ts-expect-error Deliberately test a frozen output write at runtime.
    palaces[0].stars[0].selfTransformations.inward = null;
  }, TypeError);
...
test("Ren Shen female charts preserve empty palaces, reverse decade ages and simultaneous transformations", () => {
  ...
  const expected = [
    [2, "CaiBo", 8, [42, 51], []],
    [3, "ZiNv", 9, [32, 41], ["YouBi"]],
    [4, "FuQi", 0, [22, 31], ["TianTong"]],
    [5, "XiongDi", 1, [12, 21], ["WuQu", "PoJun"]],
    [6, "Ming", 2, [2, 11], ["TaiYang"]],
    [7, "FuMu", 3, [112, 121], ["TianFu", "WenChang", "WenQu"]],
    [8, "FuDe", 4, [102, 111], ["TianJi", "TaiYin"]],
    [9, "TianZhai", 5, [92, 101], ["ZiWei", "TanLang"]],
    [10, "GuanLu", 6, [82, 91], ["JuMen"]],
    [11, "JiaoYou", 7, [72, 81], ["TianXiang", "ZuoFu"]],
    [0, "QianYi", 8, [62, 71], ["TianLiang"]],
    [1, "JiE", 9, [52, 61], ["LianZhen", "QiSha"]],
  ];
  for (const natal of [
    ...
    assert.deepEqual(
      stars
        .filter((s) => s.birthTransformation !== null)
        .map((s) => [s.name, s.birthTransformation]),
      [
        ["WuQu", "D"],
        ["ZiWei", "B"],
        ["ZuoFu", "C"],
    ...
    for (const [name, inward, outward] of [
      ...
      assert.deepEqual(stars.find((s) => s.name === name)?.selfTransformations, {
        inward,
        outward,
      });
...
test("every star projection owns its nullable fields and exposes only frozen plain data", () => {
  ...
  for (const natal of [
    ...
    const stars = [
      ...natal.palaces.flatMap((p) => p.stars),
      ...natal.palace(2).stars,
      natal.star("ZiWei"),
      natal.palaceStar(natal.ziweiPalace().branch, "ZiWei"),
...
test("all palace projections retain named, frozen own data properties", () => {
  ...
  const palaces = [
    ...
    natal.ziweiPalace(),
    natal.palace(2),
    natal.palaceByName("Ming"),
    natal.palaceByStar("ZiWei"),
    natal.oppositePalace(2),
    ...natal.sanfangPalaces(2, true),
    ...natal.sizhengPalaces(2),
    natal.decadePalaceByName(11, "Ming"),
    natal.yearlyPalaceByName(11, 9, "Ming"),
  ...
  try {
    for (const key of keys)
      Object.defineProperty(Array.prototype, key, {
        value: "unexpected inherited value",
        writable: true,
        configurable: true,
      });
