...
test('inherited code properties cannot turn valid query data into domain errors', () => {
  ...
  try {
    ...
    if (previous === undefined) delete Object.prototype.code;
...
test('relative palaces and transformation queries preserve core order and all simultaneous facts', () => {
  for (const natal of charts()) {
    ...
    assert.deepEqual(natal.sanfangPalaces(2, false).map(p => p.branch), [6, 10, 8]);
    assert.deepEqual(natal.sanfangPalaces(2, true).map(p => p.branch), [2, 6, 10, 8]);
    assert.deepEqual(natal.sizhengPalaces(11).map(p => p.branch), [11, 3, 7, 5]);
    assert.deepEqual([natal.mingPalace(), natal.shenPalace(), natal.originPalace(), natal.ziweiPalace()].map(p => p.branch), [2, 2, 10, 2]);
    assert.deepEqual(natal.birthTransformations().map(v => [v.palace.branch, v.star.name, v.star.birthTransformation]), [
      [6, 'LianZhen', 'A'], [0, 'PoJun', 'B'], [10, 'WuQu', 'C'], [11, 'TaiYang', 'D'],
    ]);
    assert.deepEqual(natal.selfTransformations().map(v => [v.palace.branch, v.star.name]), [
      [2, 'ZiWei'], [3, 'TaiYin'], [4, 'TanLang'], [6, 'LianZhen'],
      [9, 'TianTong'], [10, 'WuQu'], [10, 'YouBi'], [1, 'TianJi'],
    ]);
  ...
  const ren = Ziwei.fromBirth({ gender: 0, birthYear: 1992, birthMonth: 8, birthDay: 17, birthHour: 3 });
...
test('every palace and star identity is queryable against the independent fixed fixture', () => {
  ...
  const names = [['Ming', 2], ['XiongDi', 1], ['FuQi', 0], ['ZiNv', 11], ['CaiBo', 10], ['JiE', 9], ['QianYi', 8], ['JiaoYou', 7], ['GuanLu', 6], ['TianZhai', 5], ['FuDe', 4], ['FuMu', 3]];
  for (const natal of charts()) {
    ...
    for (const [name, branch, birth, inward, outward] of fixture) {
      const star = natal.star(name);
      assert.equal(star.name, name);
      assert.equal(natal.palaceByStar(name).branch, branches[branch]);
      assert.equal(star.birthTransformation, birth === '-' ? null : birth);
      assert.deepEqual(star.selfTransformations, { inward: inward === '-' ? null : inward, outward: outward === '-' ? null : outward });
      assert.deepEqual(natal.palaceStar(branches[branch], name), star);
    }
  }
});
...
test('query arguments retain exact paths, numeric reasons and structured domain failures', () => {
  const natal = Ziwei.fromBirth(birth);
  const calls = [
    ['palace', [2], ['branch']], ['oppositePalace', [2], ['branch']],
    ['sanfangPalaces', [2, true], ['branch', 'includeSelf']], ['sizhengPalaces', [2], ['branch']],
    ['palaceByName', ['Ming'], ['name']], ['palaceByStar', ['ZiWei'], ['name']],
    ['star', ['ZiWei'], ['name']], ['palaceStar', [2, 'ZiWei'], ['branch', 'name']],
    ['palaceTransformation', [2, 'A'], ['sourceBranch', 'kind']],
    ['palaceTransformations', [2], ['sourceBranch']], ['palaceTransformationSources', [2], ['targetBranch']],
    ['periodIndicesAtAge', [6], ['age']], ['decade', [0], ['index']],
    ['decadeByBranch', [0, 2], ['decade', 'branch']], ['decadePalaceByName', [0, 'Ming'], ['decade', 'name']],
    ['decadeYears', [0], ['decade']], ['yearly', [0, 0], ['decade', 'index']],
    ['yearlyByBranch', [0, 0, 2], ['decade', 'yearly', 'branch']],
    ['yearlyPalaceByName', [0, 0, 'Ming'], ['decade', 'yearly', 'name']],
  ];
  const failure = (call, path, reason, value) => assert.throws(call, error => {
    ...
    if (typeof value === 'number') assert.ok(Object.is(error.detail.received.value, value));
    return true;
  });
  for (const [method, args, paths] of calls) {
    for (let i = 0; i < args.length; i++) {
      failure(() => natal[method](...args.slice(0, i)), paths[i], 'missing');
      for (const value of [undefined, null, {}, [], 1n, Symbol('invalid'), () => 2]) {
        const invalid = args.with(i, value);
        failure(() => natal[method](...invalid), paths[i], 'type', value);
      }
      if (typeof args[i] === 'number') {
        for (const [value, reason] of [[NaN, 'non_finite'], [Infinity, 'non_finite'], [-Infinity, 'non_finite'], [1.5, 'non_integer'], [-1, 'out_of_range'], [256, 'out_of_range'], [4294967296, 'out_of_range']]) {
          failure(() => natal[method](...args.with(i, value)), paths[i], reason, value);
        }
        failure(() => natal[method](...args.with(i, '2')), paths[i], 'type');
      } else if (typeof args[i] === 'string') {
        for (const value of ['', 'constructor', 'toString', '__proto__', '紫微']) {
          failure(() => natal[method](...args.with(i, value)), paths[i], 'not_member');
        }
        failure(() => natal[method](...args.with(i, 123)), paths[i], 'type', 123);
      } else {
        failure(() => natal[method](...args.with(i, 1)), paths[i], 'type', 1);
      }
    }
    assert.throws(() => natal[method].apply({}, args), error => error instanceof TypeError && !(error instanceof ZiweiError));
    assert.throws(() => natal[method].call({}), error => error instanceof TypeError && !(error instanceof ZiweiError));
  }
  ...
  ]) assert.throws(() => natal[method](...args), error => {
    assert.ok(error instanceof ZiweiError);
  ...
  const hostile = { valueOf() { throw sentinel; }, toString() { throw sentinel; }, [Symbol.toPrimitive]() { throw sentinel; } };
  failure(() => natal.palace(hostile), 'branch', 'type');
  failure(() => natal.star(hostile), 'name', 'type');
});
...
test('single queries do not require whole-chart snapshots, do not cache failures and return deeply readonly data', () => {
  ...
  try {
    Object.freeze = value => {
      if (Array.isArray(value) && value.length === 12 || value && ('birthYear' in value || value.name === 'ZiWei')) throw sentinel;
      return freeze(value);
    };
    assert.throws(() => natal.palaces, error => error === sentinel);
    assert.throws(() => natal.profile, error => error === sentinel);
    assert.equal(natal.star('WuQu').birthTransformation, 'C');
    ...
    assert.throws(() => natal.star('ZiWei'), error => error === sentinel);
  } finally {
  ...
  for (const result of [natal.birthTransformations(), natal.selfTransformations(), natal.palaceTransformations(2), natal.palaceTransformationSources(0), natal.sanfangPalaces(2, true), natal.sizhengPalaces(2), natal.decade(0), natal.yearly(0, 0), natal.decadeYears(0), natal.periodIndicesAtAge(6)]) deepFrozen(result);
});
