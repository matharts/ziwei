...
test('every palace and star identity is queryable against the independent fixed fixture', () => {
  ...
  const names = [['Ming', 2], ['XiongDi', 1], ['FuQi', 0], ['ZiNv', 11], ['CaiBo', 10], ['JiE', 9], ['QianYi', 8], ['JiaoYou', 7], ['GuanLu', 6], ['TianZhai', 5], ['FuDe', 4], ['FuMu', 3]] as const;
  for (const natal of charts()) {
...
test('query arguments retain exact paths, numeric reasons and structured domain failures', () => {
  ...
  const failure = (call: () => unknown, path: string, reason: unknown, value?: unknown) => assert.throws(call, error => {
  ...
  for (const [method, args, paths] of calls) {
    for (let i = 0; i < args.length; i++) {
      ...
      if (typeof args[i] === 'number') {
        for (const [value, reason] of [[NaN, 'non_finite'], [Infinity, 'non_finite'], [-Infinity, 'non_finite'], [1.5, 'non_integer'], [-1, 'out_of_range'], [256, 'out_of_range'], [4294967296, 'out_of_range']]) {
          failure(() => invoke(natal[method], natal, ...args.with(i, value)), paths[i], reason, value);
        }
        failure(() => invoke(natal[method], natal, ...args.with(i, '2')), paths[i], 'type');
      } else if (typeof args[i] === 'string') {
        for (const value of ['', 'constructor', 'toString', '__proto__', '紫微']) {
          failure(() => invoke(natal[method], natal, ...args.with(i, value)), paths[i], 'not_member');
        }
    ...
    assert.throws(() => invoke(natal[method], {}, ...args), error => error instanceof TypeError && !(error instanceof ZiweiError));
    assert.throws(() => invoke(natal[method], {}), error => error instanceof TypeError && !(error instanceof ZiweiError));
  }
  ...
  ] as const) assert.throws(() => invoke(natal[method], natal, ...args), error => {
    assert.ok(error instanceof ZiweiError);
...
test('single queries do not require whole-chart snapshots, do not cache failures and return deeply readonly data', () => {
  ...
  try {
    Object.freeze = <T>(value: T): Readonly<T> => {
      if (Array.isArray(value) && value.length === 12 || value && typeof value === 'object' && ('birthYear' in value || 'name' in value && value.name === 'ZiWei')) throw sentinel;
      return freeze(value);
    };
