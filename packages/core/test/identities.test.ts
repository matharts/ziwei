...
test('identity helpers are frozen, receiver-independent and preserve the core protocol order', () => {
  ...
  assert.deepEqual(api.PalaceName.ALL, ['Ming', 'XiongDi', 'FuQi', 'ZiNv', 'CaiBo', 'JiE', 'QianYi', 'JiaoYou', 'GuanLu', 'TianZhai', 'FuDe', 'FuMu']);
  assert.deepEqual(api.StarName.ALL, ['ZiWei', 'TianJi', 'TaiYang', 'WuQu', 'TianTong', 'LianZhen', 'TianFu', 'TaiYin', 'TanLang', 'JuMen', 'TianXiang', 'TianLiang', 'QiSha', 'PoJun', 'ZuoFu', 'YouBi', 'WenChang', 'WenQu']);
  assert.deepEqual(api.Transformation.ALL, ['A', 'B', 'C', 'D']);
  assert.deepEqual(api.YinYang, { Yin: 0, Yang: 1 });
  assert.deepEqual(api.FiveElement, { Water: 'Water', Wood: 'Wood', Metal: 'Metal', Earth: 'Earth', Fire: 'Fire' });
  for (const value of [api.YinYang, api.FiveElement, api.Stem.ALL, api.Branch.ALL, api.PalaceName.ALL, api.StarName.ALL, api.Transformation.ALL]) assert.ok(Object.isFrozen(value));
