...
function fails(call: () => unknown, expected: Record<string, unknown>) {
  assert.throws(call, error => {
    ...
    for (const [key, value] of Object.entries(expected)) assert.deepEqual(Reflect.get(error.detail, key), value);
    return true;
  });
}
...
test('core domain failures retain structured payloads and declaration order', () => {
  for (const value of [0, 13, 31, 255]) {
    fails(() => invoke(Ziwei.fromBirth, undefined, { ...birth, birthMonth: value, birthHour: Infinity }), {
      code: 'INVALID_LUNISOLAR_MONTH', value,
    });
  }
  for (const value of [0, 31, 255]) {
    fails(() => invoke(Ziwei.fromBirth, undefined, { ...birth, birthDay: value }), { code: 'INVALID_LUNISOLAR_DAY', value });
  }
  fails(() => invoke(Ziwei.fromParameters, undefined, { ...parameters, birthBranch: 1 }), {
    code: 'INVALID_SEXAGENARY_YEAR', stem: 0, branch: 1,
  });
  fails(() => invoke(Ziwei.fromParameters, undefined, { ...parameters, birthBranch: 1, birthMonth: 0 }), {
  ...
  for (const [field, value] of [['gender', 2], ['birthYear', 2147483648], ['birthYear', -2147483649], ['birthHour', 12]]) {
    fails(() => invoke(Ziwei.fromBirth, undefined, { ...birth, [field]: value }), { code: 'INVALID_ARGUMENT', path: [field], reason: 'out_of_range' });
  }
  for (const [field, value] of [['birthStem', 10], ['birthBranch', 12], ['ziweiBranch', 12]]) {
    fails(() => invoke(Ziwei.fromParameters, undefined, { ...parameters, [field]: value }), { code: 'INVALID_ARGUMENT', path: [field], reason: 'out_of_range' });
  }
});

test('shape validation distinguishes missing values, rejects unknown keys and never calls accessors', () => {
  fails(() => invoke(Ziwei.fromBirth, undefined, ), { code: 'INVALID_ARGUMENT', path: [], reason: 'missing' });
  for (const input of [undefined, null, [], new Date(), '1984', 1, () => birth]) {
    fails(() => invoke(Ziwei.fromBirth, undefined, input), { code: 'INVALID_ARGUMENT', path: input instanceof Date ? ['gender'] : [], reason: input instanceof Date ? 'missing' : 'type' });
  }
  const inherited = Object.create(birth);
  fails(() => invoke(Ziwei.fromBirth, undefined, inherited), { path: ['gender'], reason: 'missing' });
  let calls = 0;
  const accessor = { ...birth, get birthYear() { calls++; return 1984; } };
  fails(() => invoke(Ziwei.fromBirth, undefined, accessor), { path: ['birthYear'], reason: 'accessor', received: { type: 'unread' } });
  const extras = { ...birth, z: 1, 'a.b': 2, get a() { calls++; return 3; } };
  fails(() => invoke(Ziwei.fromBirth, undefined, extras), { path: ['a'], reason: 'unknown_field', received: { type: 'unread' } });
  fails(() => invoke(Ziwei.fromBirth, undefined, { ...birth, [Symbol('extra')]: 1 }), { path: [], reason: 'unknown_field' });
  fails(() => invoke(Ziwei.fromParameters, undefined, { ...parameters, birthDay: 6 }), { path: ['birthDay'], reason: 'unknown_field' });
...
test('capture reads each descriptor once and propagates proxy exceptions unchanged', () => {
  ...
  assert.throws(() => invoke(Ziwei.fromBirth, undefined, new Proxy(birth, {
...
test('unknown fields keep lexical precedence over symbols regardless of own-key order', () => {
  for (const [create, valid] of [[Ziwei.fromBirth, birth], [Ziwei.fromParameters, parameters]] as const) {
    for (const keys of [['z', 'a', '10', '2'], ['😀', '\uffff', ''], ['😀', '\uffff']]) {
      ...
      fails(() => invoke(create, undefined, proxy), { path: [expected], reason: 'unknown_field' });
