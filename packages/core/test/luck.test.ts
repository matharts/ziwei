...
test('luck queries preserve natal roles and expose selected periods without requiring a numeric year', () => {
  ...
  for (const natal of [Ziwei.fromBirth(birth), Ziwei.fromParameters(parameters)]) {
    assert.deepEqual(natal.decade(1).map(v => v.name), [
      'XiongDi', 'Ming', 'FuMu', 'FuDe', 'TianZhai', 'GuanLu',
      'JiaoYou', 'QianYi', 'JiE', 'CaiBo', 'ZiNv', 'FuQi',
    ]);
    assert.deepEqual(natal.decadeByBranch(1, 3), { name: 'Ming', nameHans: '大命', nameHant: '大命' });
    const palace = natal.decadePalaceByName(1, 'Ming');
    assert.equal(palace.branch, 3);
    assert.equal(palace.name, 'FuMu');
    assert.deepEqual(natal.yearly(1, 9).map(v => v.name), [
      'FuDe', 'TianZhai', 'GuanLu', 'JiaoYou', 'QianYi', 'JiE',
      'CaiBo', 'ZiNv', 'FuQi', 'XiongDi', 'Ming', 'FuMu',
    ]);
    assert.deepEqual(natal.yearlyByBranch(1, 9, 0), { name: 'Ming', nameHans: '流命', nameHant: '流命' });
  ...
  assert.deepEqual(Ziwei.fromBirth(birth).decadeYears(1).map(v => v.year), [1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008]);
  assert.deepEqual(Ziwei.fromParameters(parameters).decadeYears(1).map(v => v.year), Array(10).fill(null));
});
...
test('reverse decades, first and final indices, and numeric year extremes remain exact', () => {
  ...
  for (const [birthYear, first, last] of [
    [-2147483648, -2147483645, -2147483526], [-1, 4, 123],
    [0, 4, 123], [2147483647, 2147483650, 2147483769],
  ]) {
