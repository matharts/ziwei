...
也可使用 `mise exec -- pnpm --filter @ziweijs/core run bench`、`bench:smoke`、`test:bench`。基准 CLI 接受 `--smoke` 和 `--output <新目录>`，`--help` 必须单独使用；不提供基线登记、对比或阈值参数。

默认优先用上面的 mise 任务：任务显式通过 `mise exec -- node` 选择项目固定版本，避免 shell 的 PATH 中另一处 Node 抢先。直接使用 pnpm 脚本时，父进程 Node 仍由调用方环境选择；运行器会以 `process.execPath` 启动计时进程，并将同一 Node 所在目录置于构建子进程 PATH 首位。报告保存实际 Node/V8，不将不同运行时的结果视为同一环境；不会修改本机全局 PATH 或 Node 配置。

```sh
mise run benchmark:node:smoke -- --output /tmp/ziwei-node-smoke-new
```

默认记录目录为仓库 `target/benchmarks/node/public-<随机 ID>/`；显式相对路径以进程工作目录为准，pnpm 包脚本的工作目录是 `packages/core/`。已有目录一律拒绝，不覆盖、不合并。构建需要先按仓库说明安装 mise 工具链及 pnpm 开发依赖；本工具不会安装依赖或发布包。
...
- `build.stdout.log` / `build.stderr.log`：构建原始输出；另存 pnpm、Rust 版本命令输出。
- `record.json`：校验全部行、顺序、计数、整数范围、语料及运行时身份后生成；含协议、原始样本、统计、Node/V8/OS/CPU、进程前后内存、负载、工具链、Git revision/dirty 和命令状态。
- `fingerprints.source` 覆盖引擎／绑定／TS 源码、构建配置及锁文件；`artifact` 覆盖实际本机原生库、加载器、JS、声明和包入口；`contract` 覆盖三个基准实现文件。各自保留排序后的逐文件 SHA-256 清单与整体哈希。
...
- `suite.mjs`：固定语料、操作、采样合同与计时子进程。
- `record.mjs`：文件指纹、严格记录校验与批平均统计。
- `run.mjs`：CLI、构建、日志、前后复核及成功／失败记录。
- `bench.test.mjs`：CLI 与记录合同；包含一次真实包冒烟，以及隔离临时包中的失败测试，不混入 `test/*.test.mjs` 功能测试。
