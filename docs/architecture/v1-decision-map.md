...
| ID | 决策 | 状态 |
| --- | --- | --- |
| D-270 | Wasm `wire.rs` 采用实例级固定数组与 `OnceCell`，按十八星和十二本命宫职身份惰性保存至多 162 个静态 JS 字符串引用；名称、简称、类别与星系仍来自核心对象。私有槽位映射不依赖枚举布局，不缓存输入、四化或查询结果；输出容器逐次新建，不跨实例共享。`dispose()` 不清空实例级静态资料，公开属性成功冻结后缓存、失败可重试的合同不变。保持 Rust 核心、Node、TS 投影、公开类型及元组协议不变，不启用 `enable-interning`。 | 用户在固定数组与 `intern` 的隔离对照后确认正式落地；实现与资源边界见[Wasm 设计](wasm-adapter-design.md#静态字符串缓存)。冷启动与预热后输出分别验收，不自动晋升性能基线；未提交或推送。 |
