...
| 你要核对什么 | 阅读位置 |
| --- | --- |
| 有哪些类型和调用方式 | 下方[结论与交付物](#结论与交付物)中的声明、用例与类型合同 |
| 输入为何被接受或拒绝 | [输入与表示](#3-输入与表示)、[错误合同](#8-错误合同) |
| Rust 查询如何映射到 Node | [全部查询映射](#5-全部查询映射)，包含输出顺序与期间宫职约定 |
| 结果能否修改、是否复用、能否独立存活 | [所有权、只读与快照复用](#6-所有权只读与快照复用) |
| JSON 输出和 Worker 如何使用 | [JSON、Worker 与可移植性](#7-jsonworker-与可移植性) |
| 平台支持和验收需要哪些证据 | [包装与兼容性目标](#9-包装与兼容性目标)、[验收与完成界限](#10-验收与完成界限) |

追溯选择原因时再读[设计过程记录](#设计过程记录)；查当前源码职责见 [Rust 与适配层架构](rust-package-design.md#阅读路径)。
...
| docs/architecture/node-api | 设计声明与编译型使用合同 | 可加载的 Node 包或绑定实现 |
...
| 对象或操作 | 复用与只读约定 |
| --- | --- |
| `Natal` | 持有 Rust 核心，JS 外壳冻结；方法按需查询，调用方不能修改本命事实。 |
| `profile`、`palaces` | 首次成功读取后分别保存深层冻结的结果；同一命盘重复读取同一属性返回同一对象，两个属性互不触发。 |
| 普通查询结果 | 深层只读，按本次请求转换；不缓存，不保证与属性快照或其他查询结果引用相等。 |
| 调用方保留的子数据 | 不反向持有命盘或原生句柄，可独立存活；在同一命盘内通过宫位 `branch`、星曜 `name` 对齐事实。 |
| `toJSON()`、Worker | 快照转换与传递见[下一节](#7-jsonworker-与可移植性)；JSON 往返或结构化克隆不保留冻结状态或查询能力。 |

### 采用的实现基线

快照保存仅限表中的两个对象属性，不加入通用 Map、LRU、缓存选项或失效 API。`zodiac`、`fiveElementBureau` 为原始身份值；大限、流年、四化仍按请求范围调用核心，不预计算或缓存所有期间，也不让单次查星依赖全盘快照。
...
D-253、D-255、D-256 的 macOS arm64／Node 24 功能、错误、只读、生命周期与打包消费端结果见 [包验收记录](#历史验收与优化记录)。D-263 后续完成首批八目标的[分发验收](node-distribution-proposal.md)，其中六个原生 runner 执行完整测试，两个 musl 目标执行 Alpine 分发消费端合同；这不代表其余候选平台、最低系统版本或性能矩阵已经完成。
...
以下记录保留当时的设计与验收状态，包括尚未创建绑定包时的描述。初始设计于 2026-09-09 依据核心提交 `26d5cbeff3f88c64a5cbb4d6cec4a4fff861b428` 制定；当前合同以本文前面的主题章节为准，后续修订关系见 [V1 决策表](v1-decision-map.md)。
