...
`wire.rs` 按十八个星曜身份、十二个本命宫职身份，以固定数组中的 `OnceCell` 惰性保存 JS 字符串。每个星曜槽保存身份名、简繁名称、简繁简称、类别和星系七项；每个宫职槽保存身份名与简繁名称三项，至多持有 162 个字符串引用。名称和其他静态事实仍读取核心对象；私有穷尽映射只选择缓存槽，不依赖 Rust 枚举布局，也不复制领域资料表。

缓存归当前单线程 Wasm 实例所有，不随输入或命盘数增长、不跨实例共享，不保存出生信息、四化或其他查询结果。每次投影仍新建元组、星曜数组和年龄区间；只复用不可变字符串。公开 `profile`／`palaces` 的首次成功冻结及失败重试合同不变。

`dispose()` 释放命盘，不清空实例级静态字符串；整个实例失去引用后才具备整体回收条件，不保证 GC 时刻、线性内存缩小或 RSS 降低。162 是字符串引用槽数，不是去重后的字符串数或总内存字节数。领域身份或静态字段变化时，须同步核对槽覆盖、跨输入一致性与容量回归。

该实现用于减少预热后输出时的重复 UTF-8 解码；首次填充有成本，不能据此承诺冷启动或纯建盘更快。隔离对照后选择固定数组，不启用 wasm-bindgen `enable-interning`；性能记录仍按[基准规范](../engineering/benchmarks.md)区分冷启动、建盘和输出，不将本机未提交结果提升为正式基线。
