...
浏览器入口只负责加载与组装。Wasm Adapter 持有 Rust 核心；输入捕获、结构化错误、出生档案、星曜、宫位和四化查询结果的只读投影及必要类型通过私有 workspace 包 `@matharts/ziwei-shared` 的根入口复用，不跨包导入源码。共享包先构建独立 ESM 与声明，再由 Node、Wasm 分别内联；浏览器产物不导入 Node 入口或原生绑定，也不保留私有共享依赖。两包错误类在各自产物中独立实例化。限运投影、缓存、初始化与释放合同仍独立管理。提取时同时跑两个 Adapter 合同，不增设公开共享包、通用后端注册器或宿主探测框架。
...
Worker 回归采用固定 50,000 次建盘与释放的有界批次，分块执行，结束不依赖主线程确认；在收到开始消息后独立观察主线程定时任务进展，并核对实际完成次数。禁用 `requestAnimationFrame` 的用例验证任务进展不依赖渲染机会。这是任务调度回归，不是持续 CPU 满载下的响应延迟或渲染流畅性证明；后两项仍需独立验收。性能验收应分别记录初始化、建盘/查询、消息往返及内存数据，不直接套用 Node 纳秒基准，也不未经校准设置绝对性能阈值。主线程同步批量调用只测正确性与耗时，不宣称无阻塞。
