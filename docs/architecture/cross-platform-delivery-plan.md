...
状态：2026-09-14，PR #350 已将固定源码重建 pnpm 测试客户端及同批仿真消费合并到 `main`（`6ab3dc8`），尚未发布。ppc64le 使用重建客户端通过仿真验收，s390x 保持官方客户端；官方 pnpm 产物的启动故障未声明上游已修复。仿真不替代真机与最低系统验收。当前结论与 CI 证据见[原生平台矩阵](../engineering/native-candidate-platforms.md)。其余七个原生候选平台、Wasm 与浏览器适配均属于完整交付目标。本文区分实现与实测状态，提交、推送和发布仍以用户授权为准。
...
2026-09-14 当前结论见[原生平台矩阵](../engineering/native-candidate-platforms.md)：提交 `be772f9` 的重建测试客户端及同批仿真消费已通过，该实现经 PR #350 合并到 `main`（`6ab3dc8`），尚未发布。上述 CI 证据仍对应原提交，不作为合并提交的新增验证。ppc64le 使用重建客户端、s390x 使用官方客户端；官方 pnpm 产物的启动故障未声明上游已修复。仿真不替代真机与最低系统验收，正式八目标支持声明不变。Android／OHOS 原生绑定保持暂停。所有包仍 private，正式发布另需独立授权。下一步仍是既有未验收宿主与发布前边界，不安装 SDK、不重开暂停项。

### 2026-09-12：ppc64le pnpm 启动阻塞

以下保留 2026-09-12 的历史验收记录，不代表当前状态：`f5961ab` 的[主 CI](https://github.com/matharts/ziwei/actions/runs/34646346959) 21 项任务全部通过，s390x 双 Node／双客户端仿真消费也通过。ppc64le 的 npm 通过，但 pnpm 直接启动仍触发空地址 `SIGSEGV`，候选总体验收未通过。
