...
状态：2026-09-12，上游同步后继续校准验收前提；2026-09-11 已按用户“并发执行”要求进入实现与验证。其余七个原生候选平台、Wasm 与浏览器适配均属于完整交付目标，不是可延期项。本文区分实现与实测状态，不授权提交、推送或发布。
...
后续先在明确授权后提交并推送本轮改动，运行新增 CI，读取对应提交的真实结果；构建机使用其自身的 pnpm，不必等待所有候选宿主的客户端问题解决。随后按宿主合同补齐原生运行与独立消费验收。Android／OHOS 的具体 SDK Interface 和真实环境尚未确定，不增加占位平台包，也不用 Wasm 结果代替原生支持。
