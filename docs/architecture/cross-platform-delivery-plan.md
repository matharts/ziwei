...
状态：2026-09-12，提交 `031fa79` 的远端候选 CI 已通过；主 CI 的 Wasm 工具安装失败已有本地修复，修复后的远端验收待完成。2026-09-11 已按用户“并发执行”要求进入实现与验证。其余七个原生候选平台、Wasm 与浏览器适配均属于完整交付目标，不是可延期项。本文区分实现与实测状态，提交、推送和发布仍以用户授权为准。
...
2026-09-12 核验提交 `031fa79e507915ba13a73f1243cf2495432a76c4` 的实际结果：

| 工作流 | 结果与边界 |
| --- | --- |
| [候选 CI `34625060596`](https://github.com/matharts/ziwei/actions/runs/34625060596) | 11 个 job 全部通过：七目标核心类型检查、三项 GNU addon 交叉构建／静态审计及汇总 gate；没有候选宿主运行证据 |
| [主 CI `34625060548`](https://github.com/matharts/ziwei/actions/runs/34625060548) | 19 个 job 成功；Wasm job 在生成胶水时因 `wasm-bindgen: not found` 失败，汇总 gate 随之失败。Rust Wasm 编译已成功，但该运行未完成浏览器验收 |

Wasm 失败源于 mise 2026.9.5 的嵌套内联任务在冷缓存下未安装子任务的 `tools`。本地修复改为在胶水调用处显式执行精确版本的 `mise exec cargo:wasm-bindgen-cli@0.2.128 -- wasm-bindgen`；独立回归覆盖冷缓存安装、暖缓存复用及安装／生成失败传播。该测试替换外部安装程序，不替代真实 CLI 下载与浏览器消费；修复后的 CI 仍须按新提交核验。

用户已确认保持 Node 的 napi-rs 与浏览器的 wasm-bindgen 两条适配路径，不合并或迁移绑定。对照实验留在本地忽略目录，不进入分发包或本次提交；上述结果不改变平台支持声明。

下一步提交并推送 Wasm 修复与相关验证记录，验收新提交的主 CI。随后按宿主合同补齐候选原生运行与独立消费验收。Android／OHOS 的具体 SDK Interface 和真实环境尚未确定，不增加占位平台包，也不用 Wasm 结果代替原生支持。公共发布仍未授权。
